...
def make_env() -> tuple[pathlib.Path, dict[str, str], pathlib.Path]:
    ...
    tmux.write_text(
        ...
        '  new-session)\n'
        '    # real args: -d -s <name> <command>. Register the session only if the command stays\n'
        '    # up, or exits cleanly; a fast non-zero exit (broken daemon) is NOT registered.\n'
        '    # tmux new-session -d always returns 0 regardless -- failure is found via has-session.\n'
        '    # Portable liveness probe (no GNU `timeout`): background, sleep briefly, kill -0.\n'
        '    name=$3; run=$4\n'
        '    bash -c "$run" & rpid=$!\n'
        '    sleep 0.2\n'
        '    if kill -0 "$rpid" 2>/dev/null; then\n'
        '      keep=1; kill "$rpid" 2>/dev/null; wait "$rpid" 2>/dev/null\n'
        '    else\n'
        '      wait "$rpid"; [ "$?" -eq 0 ] && keep=1 || keep=0\n'
        '    fi\n'
        '    if [ "$keep" -eq 1 ]; then\n'
